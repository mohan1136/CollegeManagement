...
<body>
    ...
    <?php
      ...
      session_start();
      $_SESSION['status3']="login";
    ?>

    </center>
    
<div class="w3-black" style="background-color: black;">
  ...
  <button class="w3-bar-item w3-button" onclick="openCity('View')">View Deatails</button>
   <button class="w3-bar-item w3-button" onclick="openCity('Room Mates')">View Room Mates</button>
...
<div id="Room Mates" class="w3-container city" style="display: none;">
 <?php
    $conn = mysqli_connect("localhost", "root", "", "cms");
    ...
    if(mysqli_query($conn, $sql))
    {
        ...
        if (mysqli_num_rows($result) > 0)
        {
            ...
            {
                $mobile=$row["username"];
                $sql1="SELECT room FROM room_data WHERE mobile='$mobile'";
                if(mysqli_query($conn, $sql1))
                {
                    $result1= mysqli_query($conn, $sql1);
                    if (mysqli_num_rows($result1) > 0)
                    {
                        ...
                        {
                            $room=$row["room"];
                            $sql2="SELECT fname,lname,bed FROM room_data WHERE room='$room' AND mobile!='$mobile'";
                            $result2= mysqli_query($conn, $sql2);
                            if (mysqli_num_rows($result2) > 0)
                            {
                              echo '<h1>Room mates details</h1>';
                              echo '<table border="2" style="font-size:50px; margin-top:10px;">';
                              while($row = mysqli_fetch_assoc($result2))
                              {
                                      echo '<tr style="color:red;"><td>FirstName</td><td>LastName</td><td>Bed No</td></tr>';
                                      echo '<tr><td>'.$row["fname"].'</td><td>'.$row["lname"].'</td><td>'.$row["bed"].'</td></tr>';
                              }
                              echo "</table>";
                              // $sql5="DELETE FROM login1";
                              // mysqli_query($conn, $sql5);
                            }
                            else
                            {
                                $found=1;
                                echo '<h1 style="color:red;">You are the first one, no room mates are there</h1>';
                                break;
                            }
                        }
                    }
                    ...
                    {
                        echo '<h1 style="color:blue;">You are not having room</h1>';
                        break;
                    }
                }
                else
